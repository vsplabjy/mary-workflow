...
10. For `/mw-paper read <source>`:
   ...
   - otherwise edit `reading.md` into the learner-facing English document: English-first prose, original technical terms and formulas, Chinese translations in parentheses where needed, method intuition/information flow/trade-offs, and empty Open question blocks for genuinely difficult unresolved points;
   ...
   - after local completion, resolve `本科学习` -> `科研 / 项目` -> `读论文` when no user path is specified, reuse an exact existing paper page when present or create one when absent, then write a Notion body with the full English `reading.md` original first and the Chinese `reading-summary.md` guide after `## 中文概括`; remove document H1s, preserve formulas/open-question blocks, write sequentially, and fetch the result back before claiming Notion delivery. Follow `references/paper-reading-contract.md` and `skills/notion/SKILL.md` for target and protected-block rules.
