...
10. For `/mw-paper read <source>`:
   ...
   - otherwise edit `reading.md` into the learner-facing English document by applying `.mary-research/reading-profile.md` as the authoritative translation-depth rule: keep English-first prose and original technical terms/formulas, annotate only content that exceeds the learner baseline, put concise Chinese immediately after the relevant English, and use a following Chinese paragraph when the profile requires a fuller explanation of a method, derivation, assumption, formula, figure, or table; keep empty Open question blocks for genuinely difficult unresolved points;
   ...
   - after local completion, resolve `本科学习` -> `科研 / 项目` -> `读论文` when no user path is specified, reuse an exact existing paper page when present or create one when absent, write the Chinese `reading-summary.md` guide under `## 中文概括`, then create or reuse its `English original` child page for the full `reading.md` content; never place the English original in a toggle. Remove document H1s, preserve formulas/open-question blocks, write sequentially, and fetch both pages back before claiming Notion delivery. Follow `references/paper-reading-contract.md` and `skills/notion/SKILL.md` for target and protected-block rules.
