...
14. For `/mw-paper slides [paper-id]`:
   ...
   - use the generated paper-local `Makefile`: `make slide` exports the current Marp deck, while `make hypo-template` creates a separate original Hypoxanthine-LaTeX visual comparison under `build/` without replacing `slides.md`;
