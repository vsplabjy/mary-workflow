...
4. For `read <source>` (the source may be a folder containing a PDF and optional LaTeX tree):
   ...
   - otherwise refine `reading.md` according to `references/paper-reading-contract.md`, then replace the `reading-summary.md` draft with a Chinese guide grounded in both `reading.md` and `artifacts/source.md`: keep professional terms in English, explain Method in detail by its information flow and design rationale, and keep Related Work and Experiments concise;
   - create `paper-notes.md` per `references/paper-notes-contract.md`, run `complete-read`, then create/update and read back the Notion paper page under `本科学习/科研 / 项目/读论文` when the user did not specify another path. Its body must put the complete English original from `reading.md` first, then the Chinese guide from `reading-summary.md`; follow `skills/notion/SKILL.md` for schema discovery, fetch-before-write, protected blocks, sequential writes, and verification.
