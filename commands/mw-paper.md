...
9. For `slides [paper-id]`:
   ...
   - use the generated paper-local `Makefile` or the `.mary-research/Makefile` dispatcher: `make slide` creates `build/slides.pdf` from the current `slides.md` using the built-in `mary-shanghaitech-red` theme; pass `PAPER_ID=<paper-id>` at the research root when needed;
