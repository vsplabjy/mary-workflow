...
`mw_paper.py migrate-artifacts --paper-id <paper-id>`。`prepare-slides` 会额外生成 paper-local `Makefile` 和 `.mary-research/Makefile` dispatcher；可在 paper workspace 运行 `make slide`，也可在 `.mary-research/` 根目录运行 `make slide`（多 paper 时使用 `make PAPER_ID=<paper-id> slide`），两者都使用 Mary Workflow 自带的 `mary-shanghaitech-red.css`。
