    ...
    if source.get("input_kind") == "folder" and source.get("latex_entry"):
        ...
        if entry is not None:
            try:
                ...
                for number, environment in enumerate(LATEX_FIGURE_PATTERN.findall(expanded), start=1):
                    figure_id = f"Figure {number}"
                    if not any(item["figure_id"] == figure_id for item in figure_catalog):
                        continue
                    graphic = LATEX_GRAPHICS_PATTERN.search(environment)
                    if graphic is None:
                        continue
                    asset = _safe_source_asset(entry.parent, graphic.group(1).strip())
                    if asset is None:
                        continue
