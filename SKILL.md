...
12. P5 consumes the pinned VSP-Beamer runtime under `assets/beamer/`, deploys it to the target project's `.mary-research/beamer/` directory during `prepare-slides`, and writes a paper-local XeLaTeX `Makefile` plus an isolated Hypoxanthine-LaTeX comparison source. The editable artifact is `slides.tex`; it must pass claim, Figure, layout, media, and capacity lint. `complete-slides` always compiles with `latexmk -xelatex`, rejects strict log diagnostics, audits the generated PDF, and records source/PDF fingerprints and page count. `make slide` exports `build/slides.pdf`; `make hypo-template` remains a separate comparison.
...
See `references/state-contract.md` for v2.1 milestone state, `references/paper-state-contract.md` for paper state schema 1, `references/paper-notes-contract.md` for close reading, `references/summary-contract.md` for grounded summaries, `references/slides-contract.md` for P5 slide authoring, `references/quiz-contract.md` for P6 expert Q&A, `references/beamer-assets-contract.md` for the offline presentation runtime, and `references/notion-*.md` for Notion MCP and page-craft contracts.
